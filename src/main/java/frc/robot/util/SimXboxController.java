...
/**
 * A simulated Xbox controller that uses a provided {@link ControllerMapping} to translate
 * logical button/axis names (e.g., "A", "LeftX") into raw indices. This allows the same
 * simulation code to work with different physical controller layouts (Xbox, DualSense, etc.).
 *
 * <p>This class extends {@link XboxController} but overrides all input methods to read from
 * the mapping instead of the default Xbox layout. It is used by {@link CommandSimXboxController}
 * to provide a {@link edu.wpi.first.wpilibj2.command.button.CommandXboxController} compatible
